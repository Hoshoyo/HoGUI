...
void ustring_append_fmt(char* buffer, ustring* s, const char* c, va_list args);
...
// ustring_to_cstr uses calloc to allocate new string, caller must free this memory after use
...
void ustring_append_fmt(char* buffer, ustring* s, const char* fmt, va_list args) {
	...
	ustring_append_cstr(s, buffer);
}

void ustring_print(ustring s)
{
	// @TODO(psv): improve this
	for (s64 i = 0; i < s.length; ++i)
		printf("%c", s.data[i]);
}
